        ...
        if (hasActual || hasDiff) {
          results.push({
            ...
            referenceImage: hasReference
              ? path.join(entry.name, referenceFile)
              : null,
...
async function generateHtml(results: ComparisonResult[]): Promise<string> {
  ...
  const comparisons = await Promise.all(
    results.map(async (result) => ({
      ...result,
      referenceBase64: result.referenceImage
        ? await imageToBase64(result.referenceImage)
        : null,
      actualBase64: result.actualImage
        ? await imageToBase64(result.actualImage)
        : null,
      diffBase64: result.diffImage
        ? await imageToBase64(result.diffImage)
        : null,
    })),
  );
